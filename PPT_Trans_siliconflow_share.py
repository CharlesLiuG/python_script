# 1.到 https://cloud.siliconflow.cn/account/ak 申请 API Key；
# 2.选择一个免费的模型，https://siliconflow.cn/zh-cn/pricing
...
# 初始化API相关信息和文件路径
api_key = "sk-pyktrufjcqpenkmeottsxuecmvhrxoxrttqmhanexouhjxfq"
pptx_path = 'FTNT - Web3 Solution.pptx'
output_path = 'FTNT - Web3 Solution - CN.pptx'

# 初始化OpenAI客户端
client = OpenAI(api_key=api_key, base_url="https://api.siliconflow.cn/v1")

def translate_text(text, target_language='cn'):
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {'role': 'system', 'content': f"You are a professional, authentic machine translation engine. Translate the following source text to {target_language}. Output translation directly without any additional text. Don't translate IT term. Don't translate the words beginning with 'Forti'."},
...
def translate_chart(chart, target_language):
    ...
    if chart.has_title:
        chart.chart_title.text_frame.text = translate_text(chart.chart_title.text_frame.text, target_language)
...
def translate_pptx(pptx_path, target_language='cn', output_path='translated.pptx'):
...
#        translate_slide_master(slide_master, target_language)
    ...
    if prs.core_properties.title:
        prs.core_properties.title = translate_text(prs.core_properties.title, target_language)
    if prs.core_properties.subject:
        prs.core_properties.subject = translate_text(prs.core_properties.subject, target_language)

    prs.save(output_path)
    print(f"Translated PPT saved to {output_path}")

# 执行翻译
translate_pptx(pptx_path=pptx_path, target_language='cn', output_path=output_path)